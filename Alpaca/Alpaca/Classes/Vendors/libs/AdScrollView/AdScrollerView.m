...
static const NSTimeInterval imgTime=4;
...
@implementation AdScrollerView
{
  ...
  //  UIPageControl *pageControl;
    ...
    CusPageControlWithView *pageControl;
...
//        pageControl.pageIndicatorTintColor = HEX_COLOR_ALPHA(0xffffff,0.8);
        ...
        CGRect rectValue=CGRectMake((scrollView.frame.size.width-pageControlWidth)/2.0,(scrollView.frame.size.height-pageControlHeight), pageControlWidth, pageControlHeight);
        UIImage *currentImage=[UIImage imageNamed:@"guidepage_point"];
        UIImage *pageImage=[UIImage imageNamed:@"guidepage_point_annulus"];;
        
        pageControl=[CusPageControlWithView cusPageControlWithView:rectValue pageNum:_imgArr.count-2 currentPageIndex:0 currentShowImage:currentImage pageIndicatorShowImage:pageImage];
        [scrollView.superview addSubview:pageControl];
        currentPageIdx=1;
...
- (void)scrollViewDidScroll:(UIScrollView *)sender
{
   ...
   // pageControl.currentPage=(page-1);
    pageControl.indexNumWithSlide=(page-1);
...
- (void)imagePressed:(UITapGestureRecognizer *)sender
{
    ...
    if ([_delegate respondsToSelector:@selector(adScrollerViewDidClicked:)]) {
        
        NSInteger idx =sender.view.tag%1000;
        if (idx > 0) {
            idx -=1;
        }
        [_delegate adScrollerViewDidClicked:idx];
    }
