...
- (NSDictionary *)coreTextChapterAttributes{
    UIFont *font_;
    if (!self.font) {
        self.font =25;
    }
    font_ =SEMIBOLD_FONT(self.font+5);
    //  font_ =[UIFont systemFontOfSize:self.font];
    
    UIColor *color_ =self.contentColor;
    if (!color_) {
    ...
    paragraphStyle.paragraphSpacing = font_.pointSize / 2;
    // paragraphStyle.lineSpacing = font_.lineHeight / 8.0;
    // paragraphStyle.lineSpacing = (font_.lineHeight*3)/4.0;
    // paragraphStyle.paragraphSpacing =paragraphStyle.lineSpacing*2.0;
    
    paragraphStyle.alignment = NSTextAlignmentCenter;
    
    NSDictionary *dic = @{NSParagraphStyleAttributeName: paragraphStyle, NSFontAttributeName:font_,NSForegroundColorAttributeName:color_};
    return dic;
}
...
//    paragraphStyle.paragraphSpacing = font_.pointSize / 2;
    ...
    paragraphStyle.lineSpacing = font_.pointSize * temp;
    paragraphStyle.paragraphSpacing = font_.pointSize * temp;
...
- (void)render
{
    ...
    NSArray *contentArr =[_totalString componentsSeparatedByString:@"\n\n\b"];
    ...
    [attrString setAttributes:self.coreTextChapterAttributes range:NSMakeRange(0, chapterLength)];
    
    [attrString setAttributes:self.coreTextAttributes range:NSMakeRange(chapterLength, attrString.length-chapterLength)];
