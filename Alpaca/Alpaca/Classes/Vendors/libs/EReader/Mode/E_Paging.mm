...
- (void)paginate
{
    ...
    NSDictionary *strAttr = [self stringAttrWithFont:self.contentFont color:self.contentColor];
    
    
    
    NSDictionary *strChapter =[self stringChapterAttrWithFont:self.contentFont color:self.contentColor];
    
    NSInteger chapterLength =0;
    
    
    if (attrString.string.length) {
        NSArray *contentArr =[attrString.string componentsSeparatedByString:@"\n\n\b"];
        
        
        if (contentArr.count>1) { ///表示存在标题
            chapterLength =[contentArr[0] length];
        }
        
    }
   
    [attrString setAttributes:strChapter range:NSMakeRange(0, chapterLength)];
    
    [attrString setAttributes:strAttr range:NSMakeRange(chapterLength, attrString.length-chapterLength)];
   
   /// CoreText 中最重要的两个概念就是 CTFrameSetter  生产 CTFrame。
...
- (NSDictionary *)stringChapterAttrWithFont:(NSUInteger )fontSize color:(UIColor *)color{
    UIFont *font ;
    UIColor *color_=color;
    if (!fontSize) {
        font =SEMIBOLD_FONT(25);
    }else{
        font =SEMIBOLD_FONT(fontSize+5);
    }
    ...
    if (!color_) {
        color_ = HEX_COLOR_ALPHA(0x000000, 0.8);
    }
    
    return @{NSParagraphStyleAttributeName: paragraphStyle, NSFontAttributeName:font,NSForegroundColorAttributeName:color_};
    // return @{NSForegroundColorAttributeName:color_};
}

- (NSDictionary *)stringAttrWithFont:(NSUInteger )fontSize color:(UIColor *)color
{
    UIFont *font ;
    UIColor *color_=color;
    if (!fontSize) {
...
//    paragraphStyle.paragraphSpacing = font.pointSize / 2;
    ...
    paragraphStyle.lineSpacing = font.pointSize * temp;
    paragraphStyle.paragraphSpacing = font.pointSize * temp;
