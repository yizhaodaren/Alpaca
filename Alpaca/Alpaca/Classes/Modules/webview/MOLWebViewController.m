...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clickedCloseItem) name:@"goBackToRootViewController" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(login_success) name:@"NetworkHave" object:nil];
    
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(update) name:@"updatecontrollerUI" object:nil];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (self.from ==101) {
        [self.webView evaluateJavaScript:@"task()" completionHandler:^(id _Nullable response, NSError * _Nullable error) {
...
//    [self.webView loadRequest:request];
    ...
    NSLog(@"web---%@---%@",self.webView.URL,self.urlString);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"loading"]) {
        ...
        if (!self.titleString.length) {
            ...
            if (self.from !=101) {
                [self navigationLeftItemWithImageName:@"back" centerTitle:self.webView.title titleColor:HEX_COLOR(0x000000)];
            }else{
                
                if (self.webView.canGoBack) {
                    [self navigationLeftItemWithImageName:@"back" centerTitle:self.webView.title titleColor:HEX_COLOR(0x000000)];
                }else{
                    [self navigationLeftItemWithImageName:nil centerTitle:self.webView.title titleColor:HEX_COLOR(0x000000)];
                }
...
- (void)setupBackButton {
    ...
    self.navigationItem.leftBarButtonItems = @[self.backItem, negativeSpacer];
}

- (void)leftItemEvent{
    if (self.webView.canGoBack) {
        if (self.from==100) {// 来自输入好友邀请码
            [self clickedCloseItem];
        }else{
            [self.webView goBack];
            // self.backNavigation = [self.webView goBack];
...
- (void)setupProgressView
{
    ...
    self.progressView.width = KSCREEN_WIDTH;
    ...
    self.progressView.y = StatusBarAndNavigationBarHeight;
}

- (void)viewWillLayoutSubviews {
    [super viewWillLayoutSubviews];
    self.webView.width = self.view.width;
    self.webView.height = self.view.height - StatusBarAndNavigationBarHeight;
    self.webView.y = StatusBarAndNavigationBarHeight;
}
